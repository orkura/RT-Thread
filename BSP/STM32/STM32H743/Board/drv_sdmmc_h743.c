 ...
 */
/* H743-local SDMMC driver, derived from Libraries/HAL_Drivers/drivers/drv_sdmmc.c.
 * Keep experimental eMMC support here until validated for the shared driver.
 * SDMMC1 serves the onboard eMMC; SDMMC2 serves the 4-bit SD card socket.
 */
#include "board.h"
...
/* Dedicated whole cache lines per controller; the H743 linker places .bss in
 * AXI SRAM, which is accessible to both SDMMC IDMA controllers. */
#ifdef BSP_USING_SDIO1
rt_align(SDIO_ALIGN_LEN)
static rt_uint8_t sdmmc1_cache_buf[SDIO_BUFF_SIZE];
#endif
#ifdef BSP_USING_SDIO2
rt_align(SDIO_ALIGN_LEN)
static rt_uint8_t sdmmc2_cache_buf[SDIO_BUFF_SIZE];
#endif
...
static void rthw_sdio_send_command(struct rthw_sdio *sdio, struct sdio_pkg *pkg)
{
    ...
    if (data != RT_NULL)
    {
        rt_uint32_t cache_size = RT_ALIGN(data->blks * data->blksize, SDIO_ALIGN_LEN);

        /* Maintain only this host's DMA buffer, never the other controller's
         * buffer while it may be in flight. Cover full cache lines for short
         * transfers such as SCR reads as well as sector transfers. */
        if (data->flags & DATA_DIR_READ)
        {
            SCB_CleanInvalidateDCache_by_Addr(sdio->cache_buf, (int32_t)cache_size);
        }
        else
        {
            SCB_CleanDCache_by_Addr(sdio->cache_buf, (int32_t)cache_size);
        }
   ...
   if (resp_type(cmd) == RESP_NONE)
    ...
    if (data != RT_NULL)
    {
        if (data->flags & DATA_DIR_READ)
        {
            ...
            SCB_InvalidateDCache_by_Addr(sdio->cache_buf,
                                       (int32_t)RT_ALIGN(size, SDIO_ALIGN_LEN));
...
static void rthw_sdio_iocfg(struct rt_mmcsd_host *host, struct rt_mmcsd_io_cfg *io_cfg)
{
    ...
    /* H7 stops the card clock in the power-off state. Never divide by zero
     * when the core requests clock = 0 during power-up or power-off. */
    if (clk == 0U || io_cfg->power_mode == MMCSD_POWER_OFF)
    {
    ...
    /* CLKDIV = 0 bypasses the divider; otherwise CK = kernel / (2 * CLKDIV).
     * Round up so the card clock never exceeds the core's requested rate. */
    if (clk >= sdmmc_clk)
    {
        Init.ClockDiv = 0U;
    }
    ...
    {
        rt_uint64_t divisor = 2ULL * clk;
        Init.ClockDiv = (rt_uint32_t)(((rt_uint64_t)sdmmc_clk + divisor - 1U) / divisor);
        if (Init.ClockDiv > SDMMC_CLKCR_CLKDIV)
        {
            LOG_E("Requested clock is below the SDMMC divider range");
            return;
        }
    }
    hsd->Init.ClockDiv = Init.ClockDiv;
...
struct rt_mmcsd_host *sdio_host_create(struct stm32_sdio_des *sdio_des)
{
    ...
    if (sdio->cache_buf == RT_NULL)
    {
        LOG_E("No DMA buffer for this SDMMC instance");
...
#ifdef BSP_SDIO1_USING_8_BIT
    if (sdio_des->hw_sdio.Instance == SDMMC1)
    {
        host->flags |= MMCSD_BUSWIDTH_8;
    }
#endif /* BSP_SDIO1_USING_8_BIT */
#else
...
int rt_hw_sdio_init(void)
{
    ...
    /* CubeMX places the eMMC pin and clock setup in the MMC MSP callback.
     * Use the proper handle type; card enumeration remains owned by RT-Thread. */
    MMC_HandleTypeDef hmmc = {0};
